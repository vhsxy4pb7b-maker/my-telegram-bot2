...
if len(order_id) == 10:
    # 标准格式：YYMMDDNNKK
    date_part = order_id[:6]  # YYMMDD
    amount_part = int(order_id[8:10])  # KK
    amount = amount_part * 1000
    print(f"格式: 标准10位格式")
elif len(order_id) == 8:
    # 可能是：YYMMDDKK（没有序号）
    date_part = order_id[:6]  # YYMMDD
    amount_part = int(order_id[6:8])  # KK
    amount = amount_part * 1000
    print(f"格式: 8位格式（无序号）")
else:
    print(f"警告: 订单ID长度不符合标准格式")
    # 尝试解析前6位作为日期
    date_part = order_id[:6] if len(order_id) >= 6 else order_id
    amount = 0
    print(f"无法确定金额，使用默认值")
...
try:
    ...
    if amount > 0:
        print(f"金额部分: {amount_part if 'amount_part' in locals() else 'N/A'}")
...
if order_date_from_id:
    ...
    if amount > 0:
        ...
        print("\n注意: 无法确定订单金额，无法生成完整播报消息")
        print(f"但下个付款日期是: {date_str} ({weekday_str})")
